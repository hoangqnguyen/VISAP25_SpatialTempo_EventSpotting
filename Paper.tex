...
\title{MTSN: Multi-Task Temporal and Spatial Network for High-Precision Event Spotting in Volleyball Videos}
...
\keywords{Precise Event Spotting, Sports Video Understanding, Spatial Temporal Event Spotting, Deep Learning, Volleyball}
...
\abstract{Precise event spotting is crucial in applications like sports analysis, where both temporal and spatial accuracy are key, especially in fast-paced sports like volleyball. We introduce a new task: high-precision spatial-temporal event spotting in volleyball videos, requiring detection of both when and where events occur. To address this, we develop a novel volleyball dataset annotated for temporal and spatial event localization and propose a multi-task deep learning model that jointly predicts event timing and spatial coordinates. Adding spatial prediction improves temporal event detection by ???\%, demonstrating the synergy between these tasks. To the best of our knowledge, this is the first work tackling this task, establishing a strong baseline and providing a foundation for future research in spatial-temporal event spotting.}
