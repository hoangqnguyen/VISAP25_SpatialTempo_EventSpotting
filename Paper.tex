...
% \abstract{
%   Precise event spotting is crucial for sports video analysis, where understanding both the timing and location of key actions is essential, especially in fast-paced games like volleyball. We introduce a new task: high-precision spatial-temporal event spotting in volleyball videos, focusing on detecting both when and where key events occur. To address this challenge, we develop a novel volleyball dataset with annotations for temporal and spatial event localization. Our multi-task deep learning model jointly predicts event timing and spatial coordinates, and we find that adding a spatial predictor improves temporal detection by ???\%, highlighting the synergy between spatial and temporal tasks. To the best of our knowledge, this is the first work to tackle this task, setting a strong baseline for future research in spatial-temporal event spotting.
% }

% \abstract{
%   Understanding both when and where events occur is crucial for analyzing sports videos, especially in fast-paced games like volleyball. We introduce a new task: high-precision spatial-temporal event spotting, targeting the challenge of localizing key events both temporally and spatially. To achieve this, we present a novel annotated volleyball dataset and propose a multi-task deep learning model. By integrating spatial predictions, our approach improves temporal event detection by 10\%, showing a strong interaction between spatial and temporal tasks. This is the first known work to address this task, establishing a baseline for future studies.
% }

% \abstract{
%   Understanding the precise timing and location of events is critical for analyzing sports videos, especially in high-speed sports like volleyball. We introduce a new task: high-precision spatial-temporal event spotting, which focuses on detecting both the timing and location of key actions. To address this, we present the KOVO Event Dataset, consisting of 947 volleyball rally videos, 890,797 frames, and 5,935 key actions, annotated for both temporal and spatial localization. Our multi-task deep learning model jointly predicts event times and locations, and we find that incorporating spatial predictions improves temporal accuracy by 10\%. To the best of our knowledge, this is the first work to address this task, setting a strong baseline for future research in spatial-temporal event spotting.
% }

\abstract{
  Understanding the precise timing and location of events is crucial for analyzing sports videos, especially in fast-paced sports like volleyball. We introduce a new task: high-precision spatial-temporal event spotting, which aims to detect both when and where key actions occur. To support this, we present the KOVO Volleyball Event Dataset, featuring 947 rally videos, 890,797 frames, and 5,935 events, annotated for both temporal and spatial localization. Our best model achieves a temporal mAP of 90.59, a spatial mAP of 77.94, and a combined mAP of 85.46 across various temporal and spatial thresholds. Notably, we find that incorporating spatial predictions enhances mAP by 10\%, underscoring the synergy between spatial and temporal analysis. To the best of our knowledge, this is the first work addressing this task, establishing a strong baseline for future research in spatial-temporal event spotting.
}
